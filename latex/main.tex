...
\section{Implementation: Technologies and Techniques Used}
  ...
  \subsection{Hardware Entropy Source}
    ...
    The Art of Electronics~\cite[p.~984]{art_of_electronics} describes the noise produced by the circuit as being spectrally white up to around 50MHz, with a 100mVrms output signal.
...
\section{Evaluation and Characterisation} \label{characterisation_section}
  ...
  \subsection{Evaluation of Hardware Entropy Source}
    ...
    This section presents measurements taken at several test points in the assembled circuit, analysing the noise signal at each stage in the amplifier circuit. A schematic is available in Appendix \ref{appendix:schematic}, showing the locations of these test points within the circuit. The author measured and recorded voltage waveforms at each of the testpoints listed below using a Tektronix TDS 1001B oscilloscope with both AC and DC probe coupling --- DC coupling was used for the waveform screenshots shown below, as well as for measuring average voltage, whilst AC coupling was used for $V_{RMS}$ and peak-to-peak voltage $V_{pp}$ measurements. The author took measurements using a 1ms timebase, corresponding to a 250MS/s sample rate for this oscilloscope~\cite{tektronix}. A \textit{measurement} consisted of the simulataneous capture of a bitmap screenshot from the oscilloscope and a comma-separated value record of the waveform samples being displayed.

    The author used Matlab to generate discrete fourier transforms for the waveforms measured with DC probe coupling; this involved subtracting the measured mean voltage from all voltage samples before taking the fast fourier transform. No window function was applied since the noise spectrum was predicted to be fairly wide (white up to around 50MHz~\cite[p.~984]{art_of_electronics}).

    The oscilloscope records 2500 waveform data points at a time, corresponding to a frequency resolution of 100kHz for a 250MS/s sample rate. The author also decided to measure the noise output (TP9) using a 5ms (50MS/s) timebase setting with 20MHz BW limit filter enabled, to provide a greater frequency resolution for the output noise spectrum over a smaller measurement bandwidth (0 to 20MHz vs 0 t0 50MHz previously).
    ...
    \subsubsection{Results}
      ...
      \paragraph{TP2: Avalanche breakdown noise source}
        ...
        \begin{figure}[H]
          \centering
          \includegraphics[width=0.5\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
      ...
      \paragraph{TP4: Input to first op-amp}
        ...
        \begin{figure}[H]
          ...
          \includegraphics[width=0.5\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{Measured waveform with DC probe coupling. $V_{average} = $.}% TODO: fill in V_{average}
          \label{fig:tp4_waveform}
        \end{figure}

        \begin{figure}[H]
          \centering
          \includegraphics[width=0.6\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{FFT of waveform shown in Figure \ref{fig:tp4_waveform}.}
          \label{fig:tp4_fft}
        \end{figure}
      ...
      \paragraph{TP8: Output of first op-amp/input to second op-amp}
        ...
        \begin{figure}[H]
          ...
          \includegraphics[width=0.5\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{Measured waveform with DC probe coupling. $V_{average} = $.}% TODO: fill in V_{average}
          \label{fig:tp8_waveform}
        \end{figure}

        \begin{figure}[H]
          \centering
          \includegraphics[width=0.6\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{FFT of waveform shown in Figure \ref{fig:tp8_waveform}.}
          \label{fig:tp8_fft}
        \end{figure}
      ...
      \paragraph{TP9: Output of second op-amp}
        ...
        \begin{figure}[H]
          ...
          \includegraphics[width=0.5\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{Measured waveform with DC probe coupling. $V_{average} = $.}% TODO: fill in V_{average}
          \label{fig:tp9_waveform}
        \end{figure}

        \begin{figure}[H]
          \centering
          \includegraphics[width=0.6\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{FFT of waveform shown in Figure \ref{fig:tp9_waveform}.}
          \label{fig:tp9_fft}
        \end{figure}
      ...
      \paragraph{TP10: Circuit noise output}
        ...
        \begin{figure}[H]
          ...
          \includegraphics[width=0.5\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{Measured waveform with DC probe coupling. $V_{average} = $.}% TODO: fill in V_{average}
          \label{fig:tp10_waveform}
        \end{figure}

        \begin{figure}[H]
          \centering
          \includegraphics[width=0.6\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{FFT of waveform shown in Figure \ref{fig:tp10_waveform}.}
          \label{fig:tp10_fft}
        \end{figure}
        ...
        \begin{figure}[H]
          \centering
          \includegraphics[width=0.6\textwidth]{fig/PCB_CROPPED.jpg}% TODO: fill in
          \caption{FFT of data sampled at 50MS/s with 20MHz BW limit filter applied.}
          \label{fig:tp10_fft_lowfreq}
        \end{figure}

      \textit{$\langle$ TODO: quantify randomness of output, with and without avalanche source$\rangle$}

    \subsubsection{Discussion}

      \textit{$\langle$ TODO: remove this?:$\rangle$}
