...
\usepackage{epsfig}
\usepackage{amsmath,graphicx,psfrag,pstricks,float,amssymb,fancyhdr,pdfpages, hyperref, enumitem, listings, minted, diagbox, appendix, lastpage, biblatex, bm, subfig}
...
\begin{appendix}
  ...
  \section{Derivation of an Upper Bound on \textit{Indirect} Differential Privacy Loss}
    ...
    \end{align}
    where $p()$ denotes the probability density function for a random variable. I have been unable to insert this value into the privacy loss equation, however the following equation can be obtained if the conditional distribution of $\hat{X}$ given $\hat{Y}$ is known:
